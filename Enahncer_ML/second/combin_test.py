...
if __name__=="__main__":
    for i in xrange(len(input_files)):
        for j in xrange(i+1,len(input_files)):
            ...
            scaler = joblib.load('train_second_2-4mer_second_3proba_scaler.pkl')
            output_file = scaler.transform(np.array(output_file))
            #joblib.dump(scaler, output_name.split('.')[0]+'_scaler.pkl')
            print "normalization"
